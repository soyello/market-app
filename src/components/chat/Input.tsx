...
const sendRequest = (
  url: string,
  { arg }: { arg: { text: string; image: string; receiverId: string; senderId: string } }
) => {
  return fetch(url, {
    method: 'POST',
...
const Input = ({ receiverId, currentUserId }: InputProps) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (message || imageUrl) {
      try {
        trigger({ text: message, image: imageUrl, receiverId: receiverId, senderId: currentUserId });
      } catch (error) {
        console.error(error);
      }
    }
  ...
  return (
    ...
    >
      ...
      <button
        type='submit'
        className='flex items-center justify-center p-2 text-gray-900 bg-slate-500 rounded-lg cursor-pointer hover:bg-slate-600 disabled: opacity-60'
      >
